...
export const postLoginRedirect = (req, res) => {
  var destination = "/" + req.originalUrl.split("/login/redir/").pop();
  res.json({
...
export const postSignup = (req, res, next) => {
  firebaseSignup.authenticate('signup', (err, user, info) => {
...
export const postInvite = (req, res) => {
  ...
  invitedRef.where("email", "==", email).then((doc) =>{
if(doc.size != 0){
  res.json('failure')
}
else{
  ...
  /*admin.database().ref('invitedUsers').orderByChild('email').equalTo(email)
  .once('value', (snapshot) => {
    const emailCheck = snapshot.val();
    if (emailCheck) {
      res.json('failure')
    } else {
      admin.database().ref('users').orderByChild('email').equalTo(email)
      .once('value', (snap) => {
        const existingUser = snap.val();
        if (existingUser) {
          res.json('failure')
        } else {
          admin.database().ref('invitedUsers').push(postData).then(() => {
            var data = {
              to: email,
              from: 'Grace on Campus <gocwebteam@gmail.com>',
              subject: 'GOC Online Invitation',
